...
from .supportGlitch import averageAmplitudes, fit, compDif2
from .utils_general import Logger, prt_center, loadFreq, medianAndErrors, specific_ratio
from .plots import plot_correlations, plot_fitSummary


def fit_glitch(path, stars, num_of_l=1, method="FQ", n_rln=1000, rtype=None, medCov=True):
    """Fit glitch for stars in path. Star data should be stored in a text file
    named "{star_name}.txt" under directory "{star_name}" for each
    ``star_name`` in stars.
    
    Args:
        path (str): path to the directory with the star data
        stars (list): list of stars names to be fitted
        num_of_l (int): number of angular degree values to be fitted
        method (str): method to be used for fitting, one of "FQ" or "SD" which
            are the direct frequency and second difference methods respectively
        n_rln (int): number of realizations to be used for fitting
        rtype (str): type of ratio to be used for fitting, one of
            ("r01", "r10", "r02", "r010", "r012", "r102") or None.
        medCov (bool): whether to store median values and covariance matrix
    """
    ...
    for star in stars:
        ...
        print ()
        prt_center("https://github.com/kuldeepv89/glitch-fitting", 88)
        print (88 * "=")
        ...
        freq, num_of_mode, num_of_n, delta_nu = loadFreq(filename, num_of_l)
        if vmin is None:
        ...
        if method.lower() == 'sd': 
            num_of_dif2, freqDif2, icov = compDif2(
                num_of_l, 
                freq, 
                num_of_mode, 
                num_of_n
        ...
        for j in range(nfit_rln):
            Acz_rln[j], Ahe_rln[j] = averageAmplitudes(
                param_rln[j, :], 
                nu_min, 
                nu_max, 
                delta_nu=delta_nu, 
        ...
        Pcz, PczNErr, PczPErr = medianAndErrors(param_rln[:, -5])
        print (
        ...
        Phe, PheNErr, PhePErr = medianAndErrors(param_rln[:, -1])
        print (
        ...
        if medCov:
            ...
            if rtype is not None:
                norder, frq, rto = specific_ratio(freq, rtype=rtype)
            ...
            plot_correlations(cov, filename=filename)
